...
import { BlockOutlined, SettingFilled } from '@ant-design/icons';
// 引入Blockly基本对象
    ...
    "blocks": [{
      ...
      "inputs": {
        "sta1": {
          "block": {
            "type": "b_load",
            "id": "OwHIduLJQ10}zSNKgrhm",
            "inputs": {
              "sta1": {
                "block": {
                  "type": "b_student",
                  "id": "iK{ow+5JIf7z:.{A{U5w",
                  "fields": {
                    "drop1": "aru_spr",
                    "drop2": "spr"
                  },
...
function PlayGround(props) {
  ...
  // Data资源
  let [sourcemap, setSourcemap] = useState(new Map([["bgm", []], ["bcg", []], ["cover", []], ["sound", []], ["spr", []]]));
  // 设置页面是否打开
   ...
   * 加载Data文件夹
   */
  const loadData = useCallback(eve => {
    // console.log(eve)
  ...
  const generateCode = () => {
    ...
    // playground生成代码
    let areacode = javascriptGenerator.workspaceToCode(primaryWorkspace.current);
    // 组合代码
  ...
  // 从已有文件保存一个文件位置
  const selectFilepath = useCallback(e => {
    if (e.target && e.target.files) {
   ...
   * 添加对话块的语音音效
   */
  const getChatscript = useCallback(() => {
  ...
  return <>
        <span id="toolsbox">
            <Row justify={"end"}>
                ...
                <Col>
                    <Button className="settingbut" onClick={() => {
            setSettingopen(true);
        ...
        {/* 积木区 */}
        <span ref={blocklyDiv} id="blocklyDiv" onKeyUp={e => console.log(e.keyCode === 83 && e.ctrlKey)} onClick={e => {
      console.log(e);
    }} />
        <div style={{
      display: 'none'
        ...
        {/* 生成的代码 显示框 */}
        <textarea value={resultcode} spellCheck={false} style={showtool ? {} : {
      display: "none"
    }} id="rescodebox" />
        {/* 资源页 */}
        <div id="sourcemodal">
            <Modal width={"80%"} style={{
        top: '25px'
      }} title="资源浏览" open={sourcepageopen} onOk={() => {
      ...
      color: 'gray'
        ...
        <div>
            <Modal width={"80%"} style={{
        top: '25px'
      }} title="设置" open={settingopen} onOk={() => {
        setSettingopen(false);
      }} onCancel={() => {
        setSettingopen(false);
      }}>
                <SettingPage version={version} loadProject={loadProject} saveProject={saveProject} openSourcePage={openSourcePage} selectFilepath={selectFilepath} downloadCode={downloadCode} getChattxt={getChattxt} getChatscript={getChatscript} changeTheme={changeTheme} darktheme={darktheme} showtool={showtool} setShowtool={setShowtool} confirmclear={confirmclear} />
            </Modal>
        </div>
